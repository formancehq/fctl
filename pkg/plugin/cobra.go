...
func buildCommandFromSpec(spec *pluginpb.CommandSpec, loaded *LoadedPlugin) *cobra.Command {
	...
	if spec.Runnable {
		...
		opts = append(opts, fctl.WithRunE(makePluginRunE(loaded, commandPath, spec.CommandType, spec.Display)))
	}
...
func makePluginRunE(loaded *LoadedPlugin, commandPath string, cmdType pluginpb.CommandType, display *pluginpb.DisplaySchema) func(cmd *cobra.Command, args []string) error {
	return func(cmd *cobra.Command, args []string) error {
		...
		if err != nil {
			// Plugin process may have crashed — include captured stderr
			return FormatPluginCrash(loaded.Name, loaded.Version, err, loaded.CapturedStderr())
		}

		switch r := resp.Result.(type) {
		case *pluginpb.ExecuteResponse_Success:
			return handleSuccess(cmd, r.Success, outputFormat, display)
...
// If a display schema is provided and json_data is available, fctl renders centrally.
// Otherwise, falls back to rendered_text from the plugin.
func handleSuccess(cmd *cobra.Command, success *pluginpb.ExecuteSuccess, outputFormat string, display *pluginpb.DisplaySchema) error {
	// Centralized rendering: display schema + json_data available
	if display != nil && success.JsonData != "" {
		return RenderFromSchema(cmd.OutOrStdout(), success.JsonData, display, outputFormat)
	}

	// Fallback: plugin-rendered output
...
// ReplaceCommandTree replaces a parent command's children with plugin-generated
// commands from the plugin's manifest. The parent command itself is preserved
// (its Use, Short, PersistentFlags, PersistentPreRunE stay intact).
func ReplaceCommandTree(parent *cobra.Command, loaded *LoadedPlugin) error {
	manifest, err := loaded.Client.GetManifest(context.Background())
	if err != nil {
		return fmt.Errorf("failed to get manifest from plugin %s: %w", loaded.Name, err)
	}

	root := manifest.RootCommand
	if root == nil {
		return fmt.Errorf("plugin %s has no root command", loaded.Name)
	}
	...
	// Add plugin's subcommands
	for _, sub := range root.Subcommands {
		parent.AddCommand(buildCommandFromSpec(sub, loaded))
	}

	// If the plugin root is runnable, set RunE on the parent
	if root.Runnable {
		parent.RunE = makePluginRunE(loaded, root.Use, root.CommandType, root.Display)
	}
