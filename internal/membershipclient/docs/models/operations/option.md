...
```go
membershipclient.WithServerURL("https://api.example.com")
```
...
```go
membershipclient.WithTemplatedServerURL("https://{host}:{port}", map[string]string{
...
```go
membershipclient.WithServerIndex(1)
```
...
```go
membershipclient.WithClient(httpClient)
```
...
```go
membershipclient.WithSecuritySource(/* ... */)
```
...
```go
membershipclient.WithTimeout(30 * time.Second)
```
