...
func NewCreateCommand() *cobra.Command {
	...
	return fctl.NewCommand("create <file>|-",
		...
		fctl.WithShortDescription("Create a payment on the Formance platform"),
...
func (c *CreateController) Run(cmd *cobra.Command, args []string) (fctl.Renderable, error) {
	...
	if !fctl.CheckStackApprobation(cmd, "You are about to create a payment") {
	...
	//nolint:gosimple
	response, err := stackClient.Payments.V1.CreatePayment(cmd.Context(), request)
	if err != nil {
		return nil, err
	}
