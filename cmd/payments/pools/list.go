...
type ListStore struct {
	Cursor *payments.PoolsCursorCursorBase `json:"cursor"`
}
...
func NewListStore() *ListStore {
	return &ListStore{
		Cursor: &payments.PoolsCursorCursorBase{},
	}
}
...
func (c *ListController) Run(cmd *cobra.Command, args []string) (fctl.Renderable, error) {
	...
	response, err := stackClient.Payments.V1.ListPools(
		cmd.Context(),
	...
	c.store.Cursor = &response.PoolsCursor.CursorBase

	return c, nil
}

func (c *ListController) Render(cmd *cobra.Command, args []string) error {
	tableData := fctl.Map(c.store.Cursor.Data, func(bc payments.Pool) []string {
		return []string{
			bc.ID,
			bc.Name,
			func() string {
