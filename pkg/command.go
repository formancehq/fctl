...
func ResolveStackID(cmd *cobra.Command, profile Profile, organizationID string) (string, error) {
	id, err := GetSelectedStackID(cmd, profile)
	if err != nil {
		return "", err
	}
	...
	return organizationClaim.Stacks[0].ID, nil
}
...
func NewStackCommand(use string, opts ...CommandOption) *cobra.Command {
	...
	_ = cmd.RegisterFlagCompletionFunc("stack", StackCompletion)
	return cmd
}
...
func NewMembershipCommand(use string, opts ...CommandOption) *cobra.Command {
	...
	_ = cmd.RegisterFlagCompletionFunc("organization", OrganizationCompletion)
	return cmd
}
