...
func NewDelete() *cobra.Command {
	return fctl.NewCommand("delete",
		fctl.WithShortDescription("Delete a manifest"),
...
func (c *DeleteCtrl) Run(cmd *cobra.Command, _ []string) (fctl.Renderable, error) {
	...
	_, err = apiClient.DeleteManifest(cmd.Context(), id)
	if err != nil {
		return nil, err
	}
...
func (c *DeleteCtrl) Render(cmd *cobra.Command, _ []string) error {
	pterm.Success.Println("Manifest deleted", c.store.ID)
	return nil
}
