...
```go
deployserverclient.WithServerURL("https://api.example.com")
```
...
```go
deployserverclient.WithTemplatedServerURL("https://{host}:{port}", map[string]string{
...
```go
deployserverclient.WithServerIndex(1)
```
...
```go
deployserverclient.WithClient(httpClient)
```
...
```go
deployserverclient.WithSecurity(/* ... */)
```
...
```go
deployserverclient.WithSecuritySource(/* ... */)
```
...
```go
deployserverclient.WithRetryConfig(retry.Config{
    Strategy: "backoff",
...
```go
deployserverclient.WithTimeout(30 * time.Second)
```
